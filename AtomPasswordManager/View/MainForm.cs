namespace AtomPasswordManager.View
{
    public partial class MainForm : Form
    {
        ...
        private void RefreshList(bool replacePassword)
        {
            ...
            foreach (Account currentAccount in Account.Accounts)
            {
                string password;
                if (replacePassword)
                {
                ...
                ListViewItem Date = new(new string[] {
        ...
        private void AcceptedCharactersTextbox_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                Manager.AllowedCharacters = AcceptedCharactersTextbox.Text;
            }
        }
        ...
        private void CheckComplexityButton_Click(object sender, EventArgs e)
        {
            ...
            if (PasswordTextbox.Text.IndexOfAny("abcdefghijklmnopqrstuvwxyz".ToCharArray()) >= 0)
            {
                complexity += 15;
            }
            ...
            if (PasswordTextbox.Text.IndexOfAny("0123456789".ToCharArray()) >= 0)
            {
                complexity += 25;
            }

            if (PasswordTextbox.Text.IndexOfAny("!@#$%^&*()_-=+;[]{}:.,<>/?".ToCharArray()) >= 0)
            {
                complexity += 25;
            }
        ...
        private void EditAccountButton_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                Account.Accounts.Insert(listView1.SelectedItems[0].Index, new(email, password, website, notes));
                email = password = website = notes = EmailTextbox.Text = PasswordTextbox.Text = WebsiteTextbox.Text = NotesTextbox.Text = string.Empty;
                RefreshList(showToggle);
                GC.Collect();
            }
        ...
        private void AddAccountButton_Click(object sender, EventArgs e)
        {
            ...
            {
                Account.Accounts.Add(new(email, password, website, notes));
                email = password = website = notes = EmailTextbox.Text = PasswordTextbox.Text = WebsiteTextbox.Text = NotesTextbox.Text = string.Empty;
                RefreshList(showToggle);
                GC.Collect();
            }
